			...
			async doExport() {
				try {
					...
					const elemWidth: number =
						(minX < 0 ? Math.abs(minX) : 0) + maxX
					const canvasElem: HTMLElement | null =
						document.getElementById(`canvas-tree-${this.$.uid}`)
					...
					if (canvasElem) {
						...
						for (let index = 0; index < photos.length; index++) {
							const element: HTMLElement = photos[
								index
							] as HTMLElement
							const url =
